...
class ycsb_cs_hybrid_worker : public ycsb_base_worker {
  ...
  virtual void MyWork(char *) override {
    ...
    if (schedule_mode == 0) {
      ...
      LOG(FATAL) << "\n-coro_scheduler=<0|1|2|3|4>"
                    ...
                    "\n2: pipeline scheduler with admission control"
                    "\n3: dual-queue pipeline scheduler with admission control";
    }
  }
 ...
 private:
  ...
  /**
   * This pipeline scheduler has one queue with admission control.
   * There is a threshold on the number of cold transactions that the scheduler can process.
   */
  void HybridPipelineAC() {
#ifdef GROUP_SAME_TRX
    LOG(FATAL) << "Pipeline scheduler doesn't work with batching same-type transactions";
#endif
...
coldq:
        ...
        auto next_cold_queue_idx = next_free_cold_queue_idx.front();
